...
export const projects: Project[] = [
  ...
  },
  {
    ...
    description: "Personal asset dashboard built on Google Apps Script and Google Sheets: current balances and the projected path to future wealth on one screen, so the numbers replace the anxiety.",
    description_ja: "GASとGoogleスプレッドシートで作った個人向け資産管理ダッシュボード。『今の残高』から『未来の資産』までの推移を1画面で可視化し、将来の資金不安を数字に置き換える。",
  ...
  },
  {
    ...
    description: "PWA for running a volleyball team, built with Node.js, Express and PostgreSQL: event management, attendance, group chat, push notifications and Google Calendar sync, with OpenAI-assisted features.",
    description_ja: "Node.js・Express・PostgreSQLで実装したバレーボールチーム運営のPWA。イベント管理、出欠確認、グループチャット、プッシュ通知、Googleカレンダー連携に加え、OpenAIによる補助機能を備える。",
    images: [
  ...
  },
  {
    ...
    description: "Web app on Google Apps Script and the Gemini API: type a daily frustration and the model articulates it and turns it into actions you can take within five minutes.",
    description_ja: "GASとGemini APIで作った課題解決Webアプリ。日常の「モヤモヤ」を入力すると、生成AIが即座に言語化し、5分で実行できるアクションに変換する。",
    images: [
  ...
  },
  {
    ...
    description: "KPI dashboard that uses Google Sheets as the database and Google Apps Script to render weekly and monthly business metrics dynamically — designed for day-to-day operations, not a demo.",
    description_ja: "Googleスプレッドシートをデータベースに、GASで週次・月次の経営指標を動的に可視化するKPIダッシュボード。デモではなく日々の運営で使うことを前提に設計。",
